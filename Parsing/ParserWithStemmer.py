...
def stemTokens(data):
    ...
    for t in data.split():
        if (is_ascii(t)):
            stemmed_word = str(stemmer.stem(t))
        else:
            stemmed_word = t
        if stemmed_word not in stemmedText:
            stemmedText.append(stemmed_word)
    stemmedText = " ".join(stemmedText)
    # stemmedText = [t for t in data.split() if str(stemmer.stem(t)) not in stemmedText]
    # stemmedText = " ".join(stemmedText)
    return stemmedText
...
def parseData(f):
    ...
    while(line != ""):
        ...
        if "<title>" in line:
           ...
           stemmedPage = stemTokens(filteredPage)
